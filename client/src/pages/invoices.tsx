...
import { z } from "zod";
import { insertInvoiceSchema, type Invoice, type PurchaseOrder, type Supplier, type Attachment } from "@shared/schema";

function InvoiceAttachmentsCell({ invoice }: { invoice: Invoice }) {
  const [open, setOpen] = useState(false);
  const { data: attachments } = useQuery<Attachment[]>({
    queryKey: ["/api/invoices", invoice.id, "attachments"],
  });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm" data-testid={`button-invoice-attachments-${invoice.id}`}>
          <Paperclip className="h-3.5 w-3.5" /> Anhänge{count > 0 ? ` (${count})` : ""}
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader><DialogTitle>Anhänge · {invoice.invoiceNumber}</DialogTitle></DialogHeader>
        ...
        <table className="w-full text-sm min-w-[820px]">
          <thead className="bg-muted/50">
            <tr className="text-left text-xs text-muted-foreground">
              ...
              <th className="px-4 py-2.5 font-medium">Anhänge</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="px-4 py-10 text-center text-muted-foreground">Keine Rechnungen vorhanden.</td></tr>
            ) : (
              (invoices ?? []).map((inv) => (
                <tr key={inv.id} data-testid={`row-invoice-${inv.id}`}>
                  ...
                  <td className="px-4 py-3">
                    <InvoiceAttachmentsCell invoice={inv} />
                  </td>
                </tr>
              ))
            )}
